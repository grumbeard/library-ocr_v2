...
(1..24).each do
  ...
  book.content.attach(io: File.open(pdf_prefix.join(filename)), filename: 'sample_book.pdf', content_type: 'application/pdf')
  if book.content

    # Create Text version of book
    book.text = []

    PDF::Reader.open(pdf_prefix.join(filename)) do |reader|
